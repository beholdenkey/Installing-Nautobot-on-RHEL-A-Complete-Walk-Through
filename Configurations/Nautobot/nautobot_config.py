...
DATABASES = {
    "default": {
        "NAME": os.getenv("NAUTOBOT_DB_NAME", ""),  # Database name
...
CACHEOPS_REDIS = os.getenv("NAUTOBOT_CACHEOPS_REDIS",
                           parse_redis_connection(redis_database=1))
...
SECRET_KEY = os.getenv("NAUTOBOT_SECRET_KEY",
                       "=0p3$c0uvfnko##p6!p+brnc7m68u=__we%6x8r5+u@rxv0ffo")
...
CACHEOPS_DEFAULTS = {"timeout": int(
    os.getenv("NAUTOBOT_CACHEOPS_TIMEOUT", 900))}
...
CORS_ALLOW_ALL_ORIGINS = is_truthy(
    os.getenv("NAUTOBOT_CORS_ALLOW_ALL_ORIGINS", False))
...
ENFORCE_GLOBAL_UNIQUE = is_truthy(
    os.getenv("NAUTOBOT_ENFORCE_GLOBAL_UNIQUE", False))
...
RELEASE_CHECK_TIMEOUT = int(
    os.getenv("NAUTOBOT_RELEASE_CHECK_TIMEOUT", 24 * 3600))
...
SHORT_DATETIME_FORMAT = os.getenv(
    "NAUTOBOT_SHORT_DATETIME_FORMAT", "Y-m-d H:i")
